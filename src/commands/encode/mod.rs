...
pub fn encode_dataset(
    ...
    let strings = generator.into_iter().map(|s| {
...
fn encode_string(s: String, merge_map: &HashMap<(u32, u32), u32>) -> Vec<u32> {
    ...
    queue.extend(utf8.windows(2).enumerate().filter_map(|(idx, window)| {
    ...
    while let Some(merge) = queue.pop() {
        ...
        if !merge_map
            .get(&pair)
            .map_or(false, |new_token| *new_token == merge.val)
        {
            continue;
        }
