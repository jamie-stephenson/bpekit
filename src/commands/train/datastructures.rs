...
impl Block {
    pub fn new(s: String, count: i32) -> Block {
        Block {
            tokens: s.as_bytes().into_iter().map(|byte| *byte as u32).collect(),
            count: count,
        }
    }
    ...
    pub fn merge(
        ...
        while token_idx < self.tokens.len() {
            if self.tokens[token_idx] == left
                && token_idx + 1 < self.tokens.len()
                && self.tokens[token_idx + 1] == right
            {
                changes
                    .entry((left, right))
...
pub fn init(local_blocks: impl Iterator<Item = String>) -> (Vec<Block>, PairCounter) {
    // Init comms
