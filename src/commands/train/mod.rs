...
pub fn train(
    ...
    let local_blocks = generator.into_iter().map(|s| {
        // TODO: Handle Result better, don't just unwrap
        s.unwrap().downcast::<PyString>().unwrap().to_string()
    });
    ...
    let mut merges: Vec<((u32, u32), u32)> =
        Vec::with_capacity((vocab_size - current_vocab_size) as usize);
    ...
    while current_vocab_size < vocab_size {
        ...
        let changes: HashMap<(u32, u32), (i32, Vec<usize>)> = pair
            .block_ids
            .par_iter()
            .map(|&block_idx| {
                let block = &blocks[block_idx] as *const _ as *mut datastructures::Block;
                unsafe { (*block).merge(left, right, current_vocab_size, block_idx) }
            })
            .reduce(
                || HashMap::new(),
